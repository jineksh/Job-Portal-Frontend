...
export const SidebarInfo = ({ user, onEditClick, isRecruiter }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border shadow-sm group">
        <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
          <CardTitle className="text-sm font-bold uppercase tracking-wider text-slate-500">
            {isRecruiter ? "Recruiter Profile" : "About Me"}
          </CardTitle>
          <Button
            variant="ghost"
            size="icon"
        ...
        <CardContent className="space-y-4">
          <div className="relative flex gap-3 text-sm text-slate-600 bg-slate-50 p-3 rounded-lg border-l-4 border-blue-500 hover:bg-slate-100 transition-colors cursor-pointer" onClick={() => onEditClick('settings')}>
            ...
            <p className="italic">{user?.bio || (isRecruiter ? "Add company bio..." : "Add a short bio about yourself...")}</p>
          </div>
      ...
      {/* Technical Skills Card: Sirf Job Seeker ko dikhega */}
      {!isRecruiter && (
            ...
            <div className="flex flex-wrap gap-2">
              {user?.skills && user.skills.length > 0 ? (
                user.skills.map((s, index) => {
                  ...
                  return (
                    <Badge key={index} variant="secondary" className="px-2 py-1 flex gap-2 items-center bg-blue-50 text-blue-700 hover:bg-blue-100 border-none group/skill">
                      {skillName}
                      <button
                        type="button"
                        disabled={isPending}
